...
class KITTI_3CLS(imdb):
  ...
  def _load_kitti_annotation(self):
    ...
    for index in self._image_idx:
      ...
      for line in lines:
        ...
        bboxes.append([x, y, w, h, cls, _get_obj_level(obj)] + [float(item) for item in obj[1:]])
  ...
  def evaluate_detections(self, eval_dir, global_step, all_boxes):
    ...
    Args:
      ...
      all_boxes: all_boxes[cls][image] = N x 5 arrays of 
        ...
        Full shape: [CLASSES, BATCHSIZE, var(FILTERED_ANCHORS), 5]
    Returns:
      aps: array of average precisions.
      names: class names corresponding to each ap
    """
    # Create a new folder "detection_files_*step*/data"
    ...
    # Write all results of all_boxes to a textfile for every image
    ...
    # Call KITTIs evaluation script that takes the arguments:
    # Path to training images / labels
    # Path to ImageSets that specifies the train/val split
    # Path to the detection file created above
    cmd = self._eval_tool + ' ' \
    ...
    # The evaluation script stores a file "stats_*class*_ap.txt"
    # Parse APs from that file and return
    aps = []
    names = []
  ...
  # detection_file_dir: Directory "detection_files_*step*/data"
  # that contains the filtered bboxes and their class probability
  # det_error_file: output file
  def analyze_detections(self, detection_file_dir, det_error_file):
    ...
    for idx in self._image_idx:
      ...
      bboxes.sort(key=lambda x: x[-1], reverse=True) # sort bboxes by prob
    ...
    with open(det_error_file, 'w') as f:
      for idx in self._image_idx: # go through all images
        gt_bboxes = np.array(self._rois[idx]) # load all gt bboxes of image
        num_objs += len(gt_bboxes)
        detected = [False]*len(gt_bboxes)

        det_bboxes = self._det_rois[idx] # detected bboxes sorted by prob
        if len(gt_bboxes) < 1:
          continue
        ...
        for i, det in enumerate(det_bboxes):
          ...
          # Compute IOU between detection and all gt_bboxes
          ious = batch_iou(gt_bboxes[:, :4], det[:4])

          # Get the GT that maximizes IOU to detection
          max_iou = np.max(ious)
          gt_idx = np.argmax(ious)
          if max_iou > 0.1: # IOU has to be atleast 0.1
            if gt_bboxes[gt_idx, 4] == det[4]: # Check if class is correct
              if max_iou >= 0.5: # Check if IOU is atleast 0.5
                if i < len(gt_bboxes):
                  if not detected[gt_idx]: # If not already detected
                    num_correct += 1 # Correct detection
                    detected[gt_idx] = True
                  else:
                    num_repeated_error += 1 # Already detected
              else:
                if i < len(gt_bboxes):
                  num_loc_error += 1 # bbox has no sufficient IOU
                  _save_detection(f, idx, 'loc', det, det[5])
            else:
              if i < len(gt_bboxes):
                num_cls_error += 1 # Wrong class detected
                _save_detection(f, idx, 'cls', det, det[5])
          else:
            if i < len(gt_bboxes):
              num_bg_error += 1 # Wrong bbox detected
