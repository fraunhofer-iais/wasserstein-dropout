...
class imdb(object):
  ...
  def __init__(self, name, mc):
    ...
    self._use_subsample = False
    self._image_idx_subsample = []
  ...
  @property
  def image_idx(self):
    if self._use_subsample:
      return self._image_idx_subsample
    else:
  ...
  @property
  def use_subsample(self):
    return self._use_subsample

  @use_subsample.setter
  def use_subsample(self, use_subsample):
    self._use_subsample = bool(use_subsample)

  def _subsample_image_set(self, frac):
    frac = float(frac)
    assert 0. < frac <= 1., "Sample fraction needs to be in (0,1]"
    sample_size = max(1, int(len(self._image_idx) * frac))
    subsample_idx = np.random.permutation(np.arange(sample_size))
    self._image_idx_subsample = [self._image_idx[i] for i in subsample_idx]

  def _shuffle_image_idx(self):

    if self._use_subsample:
      image_idx = self._image_idx_subsample
    else:
      image_idx = self._image_idx

    self._perm_idx = [image_idx[i] for i in np.random.permutation(np.arange(len(image_idx)))]
    self._cur_idx = 0

  def save_gtrois_at(self, path):
      with open(path, 'wb') as f:
          pickle.dump(self._rois, f)
  ...
  def read_image_batch(self, shuffle=True):
    ...
    if shuffle:
      if self._cur_idx + mc.BATCH_SIZE >= len(image_idx):
        self._shuffle_image_idx()
      batch_idx = self._perm_idx[self._cur_idx:self._cur_idx+mc.BATCH_SIZE]
      self._cur_idx += mc.BATCH_SIZE
    else:
      if self._cur_idx + mc.BATCH_SIZE >= len(image_idx):
        batch_idx = image_idx[self._cur_idx:] + image_idx[:self._cur_idx + mc.BATCH_SIZE-len(image_idx)]
        self._cur_idx += mc.BATCH_SIZE - len(image_idx)
      else:
        batch_idx = image_idx[self._cur_idx:self._cur_idx+mc.BATCH_SIZE]
        self._cur_idx += mc.BATCH_SIZE

    # Subtract BGR_MEANS from BGR values, resize to width/height in mc
    images, scales = [], []
    for i in batch_idx:
  ...
  def read_batch(self, shuffle=True):
    ...
    if len(batch_idx) != mc.BATCH_SIZE: # debug
      print(len(self._perm_idx), len(image_idx), self._cur_idx, shuffle, batch_idx, mc.BATCH_SIZE)
    if len(batch_idx) > mc.BATCH_SIZE:
      batch_idx = batch_idx[:mc.BATCH_SIZE]
